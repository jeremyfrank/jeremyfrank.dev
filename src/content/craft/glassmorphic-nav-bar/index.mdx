...
Finally, the spotlight movement is preped by applying a transition to the `clip-path` property, via a `data-init` attribute set by JavaScript. Then the click handlers are added to each button, which apply new left and width values.
